...
class MoldovaScraper(BaseScraper):
    ...
    def epsg3857_to_wgs84(self, x: float, y: float):
        """Конвертирует координаты EPSG:3857 → GPS (latitude, longitude)."""
    ...
    def _process_station(self, data: dict):
        ...
        # Уникальный ID — используем IDNO (налоговый номер компании)
        source_id = data.get("idno", "")
        if not source_id:
            return
